...
namespace TrRouting
{
  ...
  private:
    ...
    std::vector<std::tuple<int,int,int,int>> accessFootpaths; // pair: accessNodeIndex, walkingTravelTimeSeconds, walkingDistanceMeters, nearest osrm node distance from point (only for acees and egress journey steps))
    std::vector<std::tuple<int,int,int,int>> egressFootpaths; // pair: egressNodeIndex, walkingTravelTimeSeconds, walkingDistanceMeters, nearest osrm node distance from point (only for acees and egress journey steps))
